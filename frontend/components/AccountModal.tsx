...
export default function AccountModal({ isOpen, onClose }: Props) {
  ...
  const { claimG$, entitlement } = useSelfVerification();
  ...
  const gVal = gRaw != null ? parseFloat(formatUnits(gRaw as bigint, 18)).toFixed(2) : '0';
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <img src={`https://api.dicebear.com/9.x/pixel-art/svg?seed=${address}`} alt="avatar"
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '16px' }}>
          <div style={{ flex: 1, padding: '14px', borderRadius: '12px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div style={{ flex: 1, padding: '14px', borderRadius: '12px', background: 'rgba(245,158,11,0.05)', border: '1px solid rgba(245,158,11,0.15)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <a href={`https://celoscan.io/address/${address}`} target="_blank" rel="noopener noreferrer" style={{
          ...
          <button
            onClick={async () => {
              ...
              setClaiming(true);
              try {
                const timeout = new Promise<void>((_, reject) => setTimeout(() => reject('timeout'), 30000));
